...
const UserProfile = () => {
  ...
  const profilePicture =
    voter?.profilePicture || "/placeholder.svg?height=128&width=128";
  const isStudent = voter?.role === "student";
  const joinedDate = new Date(
    voter?.joinedDate || Date.now()
  ).toLocaleDateString();
  const studentId = voter?.studentId || "N/A";
  const phoneNumber = voter?.mobileNumber || "N/A";
  const email = voter?.email || "N/A";

  const studentInfo = {
    name: "Solomon Gabriel",
    studentId: "CS2024001",
    email: "solomon.gabriel@university.edu",
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <Card>
            <CardContent className="p-4 text-center">
              ...
              <div className="text-sm text-gray-600">Elections Voted</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 text-center">
